...
logPath = \
    '/var/www/html/vistara_db_scripts/logs/customUpdateDataUtilizationReport_test.log'
...
def dictfetchall(cursor):
    ...
    return [dict(zip([col[0] for col in desc], row)) for row in
            cursor.fetchall()]
...
def fetchCurrentAndYesterDayForUtilReportUpdateData():
    try:
        ...
        logging.debug("methodName::fetchCurrentAndYesterDayForUtilReportUpdateData :" + str(result1))
        db.close()
        cur.close()
        return result1
    except Exception as e:
        logging.debug("Exception in fetchCurrentAndYesterDayForUtilReportUpdateData method:: =>" + str(e))
...
def fetchDetailsFromSwTicketPost(from_date, to_date):
    try:
        ...
        logging.debug("methodName::fetchDetailsFromSwTicketPost, Fetch details from sw_ticket_post :"+ str(sw_ticket_post))
        db.close()
        cur.close()
        return sw_ticket_post
    except Exception as e:
        logging.debug("Exception in fetchDetailsFromSwTicketPost method :: => " + str(e))
    ...
    try:
        ...
        for detail in details:
            if detail:
                ...
                logging.debug("methodName::insertDetailsIntoStaffPostupdateTime, Insert details into staff_postupdate_time table successfully")
    except Exception as e:
        logging.debug("Exception in insertDetailsIntoStaffPostupdateTime method:: => " + str(e))

def getUtilizationStaff(from_date, to_date):
    try:
        dbobj = DBConnections.DataBaseConn()
        db = dbobj.connetReadDB()
        cur = db.cursor()
        sql1 = "SELECT DISTINCT(staffid) FROM staff_postupdate_time WHERE dateline > " + str(from_date) + " AND dateline < " + str(to_date)
        cur.execute(sql1)
        result1 = dictfetchall(cur)
        db.close()
        cur.close()
        ...
        logging.debug('Exception in getUtilizationStaff method:: =>' + str(e))

def getUserUtilizationDataDaywise(from_date, to_date):
    try:
        day_wise_data = []
        util_staff = getUtilizationStaff(from_date, to_date)
        logging.debug("Util_Staff ==========>" + str(util_staff))
        ...
        ignore_staff_list = [2, 15757, 103, 8331, 18617, 8341, 14817, 10707, 10587, 13167, 320]
        # print staff_data
        ...
        for staff in staff_data:
            logging.debug("staff :" + str(staff))
            if staff in util_staff:
                if staff not in ignore_staff_list:
                    cond = ""
                    ...
                    try:
                        ...
                        sql += "where b.dateline >= " + str(from_date) + " and b.dateline < " + str(to_date) + " and b.staffid > 0 " + str(cond) + "and e. Timespent > 0  order by b.dateline"
                        ...
                        if result:
                            ...
                            for utilization_data in result:
                                ...
                                prepare_utilization_data['swstaff_id'] = int(utilization_data['staffid'])
                                prepare_utilization_data['ticketid'] = int(utilization_data['ticketid'])
                                prepare_utilization_data['ticketpostid'] = int(utilization_data['ticketpostid'])
                                prepare_utilization_data['resource_name'] = str(utilization_data['name'])
                                prepare_utilization_data['touched_date'] = int(utilization_data['dateline'])
                                prepare_utilization_data['timespent'] = int(utilization_data['timespent'])
                                prepare_utilization_data['billable_time'] = int(utilization_data['timebillable'])

                                # Fetch nr_staffid and his manager_id by swstaffid
                                if utilization_data['staffid']:
                                    if employee_info:
                                        for emp_info in employee_info:
                                            if emp_info['staffid'] == utilization_data['staffid']:
                                                # print "EMP INFO: ", emp_info
                                                prepare_utilization_data['nrstaff_id'] = int(emp_info['id'])
                                                reporting_mngr_id = emp_info['reporting_manager_id']

                                                if manager_info:
                                                    for mngr_info in manager_info:
                                                        # print "MNGR INFO: ", mngr_info
                                                        if mngr_info['id'] == reporting_mngr_id:
                                                            prepare_utilization_data['manager_id'] = int(mngr_info['swstaff_id'])
                                                            prepare_utilization_data['manager_name'] = str(mngr_info['staff_fullname'])

                                prepare_utilization_data['ticket_created_date'] = int(utilization_data['created_dt'])
                                prepare_utilization_data['post_date'] = int(utilization_data['dateline'])
                                prepare_utilization_data['subject'] = str(utilization_data['Subject'])
                                prepare_utilization_data['partner_id'] = int(utilization_data['mid'])
                                prepare_utilization_data['partner'] = str(utilization_data['partner'])
                                prepare_utilization_data['client_id'] = int(utilization_data['cid'])
                                prepare_utilization_data['client'] = str(utilization_data['client'])
                                prepare_utilization_data['ticket_type'] = str(utilization_data['typename'])
                                if str(utilization_data['device']) != '':
                                    prepare_utilization_data['device'] = str(utilization_data['device'])
                                else:
                                    prepare_utilization_data['device'] = 'NA'

                                if str(utilization_data['devicetype']) != '':
                                    prepare_utilization_data['devicetype'] = str(utilization_data['devicetype'])
                                else:
                                    prepare_utilization_data['devicetype'] = 'NA'

                                # print "\n Final data prepared user wise:", len(prepare_utilization_data)

                                # insertRecord(prepare_utilization_data, 'utilization_report_data')
...
def main():
    ...
    try:
        ...
        details = fetchDetailsFromSwTicketPost(tmp[0]["yesterDayPST"], tmp[0]["currentTimeInPST"])

        # if details:
        #     insertDetailsIntoStaffPostupdateTime(details)

        getUserUtilizationDataDaywise(tmp[0]['yesterDayPST'],tmp[0]['currentTimeInPST'])
...
logging.debug('---------------- Script  completed at:%s ------------------' % dt.datetime.utcnow())
